...
public class GenzongAdapter extends RecyclerView.Adapter<GenzongAdapter.ViewHolder> {
    ...
    private List<Genzong.DataListBean> list = new ArrayList<>();
    ...
    public void onReference(List<Genzong.DataListBean> list1) {
        if (list1 != null) {
            list.clear();
            list.addAll(list1);
    ...
    public void addOnReference(List<Genzong.DataListBean> list1) {
        if (list1 != null) {
            list.addAll(list1);
            notifyDataSetChanged();
        }
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        ...
        Genzong.DataListBean dataBean = list.get(position);
        ...
        Glide.with(context)
                .load(Constant.Image_Header + dataBean.getActivitieType().getImgPath())
